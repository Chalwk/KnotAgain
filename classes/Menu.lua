...
local helpText = {
    "KNOT AGAIN!",
    "Untangle all ropes by moving the nodes!",
    "",
    "HOW TO PLAY:",
    "• Click and drag nodes to move them around",
    "• Fixed nodes (with X) cannot be moved",
    "• Untangle ropes until no crossings remain",
...
function Menu:draw(screenWidth, screenHeight, state)
    ...
    if state == "menu" then
        if self.showHelp then
            ...
            lg.setFont(self.smallFont)
            lg.printf("KNOT AGAIN!\nREPLACE_THIS_TEST", 0, screenHeight / 3 + 30, screenWidth, "center")
